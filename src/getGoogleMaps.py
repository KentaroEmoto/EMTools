...
def __get_image(x: int, y: int, zoom: int, map_type: str) -> Image.Image:
    """Download tile image.
...
def create_image(out_image_name: str, lon1: float, lat1: float, lon2: float, lat2: float, zoom: int = None, map_type: str = 'm'):
    ...
    Args:
        ...
        map_type (str, optional): Tile ID of the map type. 'Road:h', 'Terrain:t', 'Hybrid:y', 'Satellite:s', 'Street:r' and 'Maps:m'. (See https://ujicya.jp/blog-mapping/xyz-tiles-url/). Defaults to 'm'.
    """
    if zoom is None:
    ...
    # left top
    img1 = __get_image(x_l, y_t, zoom, map_type).crop((x_l_px, y_t_px, x_max, y_max))
    img_comb.paste(img1, (0, 0))
    # left bottom
    img1 = __get_image(x_l, y_b, zoom, map_type).crop((x_l_px, y_min, x_max, y_b_px))
    img_comb.paste(img1, (0, (n_tile_y-1)*256-y_t_px+y_min))
    # right top
    img1 = __get_image(x_r, y_t, zoom, map_type).crop((x_min, y_t_px, x_r_px, y_max))
    img_comb.paste(img1, ((n_tile_x-1)*256-x_l_px+x_min, 0))
    # right bottom
    img1 = __get_image(x_r, y_b, zoom, map_type).crop((x_min, y_min, x_r_px, y_b_px))
    img_comb.paste(img1, ((n_tile_x-1)*256-x_l_px+x_min, (n_tile_y-1)*256-y_t_px+y_min))

    for i in range(n_tile_x-2):
        # top
        img1 = __get_image(x_l+i+1, y_t, zoom, map_type).crop((0, y_t_px, 256, y_max))
        img_comb.paste(img1, ((i+1)*256-x_l_px, 0))
        # bottom
        img1 = __get_image(x_l+i+1, y_b, zoom, map_type).crop((0, y_min, 256, y_b_px))
        img_comb.paste(img1, ((i+1)*256-x_l_px, (n_tile_y-1)*256-y_t_px+y_min))
    for i in range(n_tile_y-2):
        # left
        img1 = __get_image(x_l, y_t+i+1, zoom, map_type).crop((x_l_px, 0, x_max, 256))
        img_comb.paste(img1, (0, (i+1)*256-y_t_px))
        # right
        img1 = __get_image(x_r, y_t+i+1, zoom, map_type).crop((x_min, 0, x_r_px, 256))
        img_comb.paste(img1, ((n_tile_x-1)*256-x_l_px+x_min, (i+1)*256-y_t_px))

    for i in range(n_tile_x-2):
        for j in range(n_tile_y-2):
            img1 = __get_image(x_l+i+1, y_t+j+1, zoom, map_type)
